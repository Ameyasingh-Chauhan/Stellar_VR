...
# New function to create final equirectangular stereo frames
def create_final_panoramic_stereo(left_dir, right_dir, output_dir, progress_cb=None):
    """
    Create final equirectangular panoramic stereo frames from processed left/right views
    ...
    Optimized for speed:
    - Multi-threading for parallel processing
    - Frame skipping for ultra-fast mode
    - Reduced processing for similar frames
    """
    ensure_dir(output_dir)
    left_files = sorted(Path(left_dir).glob('*.png'))
    right_files = sorted(Path(right_dir).glob('*.png'))
    ...
    print(f"[PanoramicProjection] Creating equirectangular stereo frames: {n} frames")
    print(f"  - Output format: {TARGET_PER_EYE*2}x{TARGET_PER_EYE} (VR180)")
    print(f"  - Left/right stereo side-by-side")
    print(f"  - 180° horizontal FOV")
    
    # For ultra-fast mode, process every 3rd frame and interpolate others (more frequent than before)
    PROCESS_EVERY_NTH = 3 if ULTRAFAST_MODE else 1
    
    # Filter files for frame skipping in ultra-fast mode
    if PROCESS_EVERY_NTH > 1:
        left_files_filtered = left_files[::PROCESS_EVERY_NTH]
        right_files_filtered = right_files[::PROCESS_EVERY_NTH]
        print(f"  - Ultra-fast mode: Processing every {PROCESS_EVERY_NTH}th frame ({len(left_files_filtered)} frames)")
    else:
        left_files_filtered = left_files
        right_files_filtered = right_files
        print(f"  - Normal mode: Processing all {n} frames")
    
    # Keep track of processed frames for interpolation
    processed_frames = {}
    
    def _process_single_panoramic_frame(i, l, r):
        """Process a single panoramic stereo frame"""
        if i % 10 == 0:  # Print progress every 10 frames
            print(f"  Processing frame {i+1}/{n}: {l.name}")
        ...
        # Create equirectangular panoramic stereo frame
        # Output: based on TARGET_PER_EYE setting
        stereo_frame = create_equirectangular_panorama(L_aligned, R_aligned, output_height=TARGET_PER_EYE)
        
        # Save the final equirectangular stereo frame
        output_path = Path(output_dir) / f"panoramic_stereo_{i:06d}.png"
        cv2.imwrite(str(output_path), stereo_frame)
        
        # Store for interpolation in ultra-fast mode
        processed_frames[i] = str(output_path)
        return i, str(output_path)
    ...
    if workers > 1 and len(left_files_filtered) > 1:
        print(f"  - Using {workers} worker threads for parallel processing")
        # Use multi-threading for parallel processing
        with ThreadPoolExecutor(max_workers=workers) as executor:
            # Submit all panoramic processing tasks
            future_to_index = {
                executor.submit(_process_single_panoramic_frame, i, l, r): i 
            ...
            for future in as_completed(future_to_index):
                ...
                try:
                    ...
                    completed += 1
                    if progress_cb and (completed % 5) == 0:
                        progress_cb(45 + int(10 * (completed / max(1, len(left_files_filtered)))), f'Panoramic Stereo {completed}/{len(left_files_filtered)}')
                except Exception as exc:
                    print(f'Frame {index} generated an exception: {exc}')
    else:
        print("  - Using single-threaded processing")
        ...
        for i, (l, r) in enumerate(zip(left_files_filtered, right_files_filtered)):
            try:
                result_index, result_path = _process_single_panoramic_frame(i, l, r)
                if progress_cb and (i % 5) == 0:
                    progress_cb(45 + int(10 * (i / max(1, len(left_files_filtered)))), f'Panoramic Stereo {i}/{len(left_files_filtered)}')
            except Exception as exc:
                print(f'Frame {i} generated an exception: {exc}')
    
    # In ultra-fast mode, interpolate skipped frames
    if PROCESS_EVERY_NTH > 1 and processed_frames:
        print("  - Interpolating skipped frames for ultra-fast mode")
        for i in range(n):
            if i not in processed_frames:
                # Find nearest processed frames for interpolation
                prev_processed = None
                next_processed = None
                
                # Find previous processed frame
                for j in range(i-1, -1, -1):
                    if j in processed_frames:
                        prev_processed = j
                        break
                
                # Find next processed frame
                for j in range(i+1, n):
                    if j in processed_frames:
                        next_processed = j
                        break
                
                # Interpolate or copy nearest frame
                if prev_processed is not None and next_processed is not None:
                    # Average of two nearest frames (simplified interpolation)
                    prev_frame_path = processed_frames[prev_processed]
                    next_frame_path = processed_frames[next_processed]
                    # For simplicity, just copy the previous frame
                    output_path = Path(output_dir) / f"panoramic_stereo_{i:06d}.png"
                    import shutil
                    shutil.copy(prev_frame_path, str(output_path))
                elif prev_processed is not None:
                    # Copy previous frame
                    prev_frame_path = processed_frames[prev_processed]
                    output_path = Path(output_dir) / f"panoramic_stereo_{i:06d}.png"
                    import shutil
                    shutil.copy(prev_frame_path, str(output_path))
                elif next_processed is not None:
                    # Copy next frame
                    next_frame_path = processed_frames[next_processed]
                    output_path = Path(output_dir) / f"panoramic_stereo_{i:06d}.png"
                    import shutil
                    shutil.copy(next_frame_path, str(output_path))
    
    print(f"[PanoramicProjection] Created {n} equirectangular stereo frames")
